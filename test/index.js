...
describe('HTML Codesniffer runner', function main() {
  ...
  describe('standards', () => {
    ...
    it('should be possible to pass multiple standards to test', () => {
      const crawler = new CrawlKit(url);
      crawler.addRunner('htmlcs', new HtmlCsRunner(), [
        HtmlCsRunner.standard.Section508,
        HtmlCsRunner.standard.WCAG2A,
      ]);
  ...
  describe('level threshold', () => {
    it('should be possible to filter messages based on a given level', () => {
      ...
      crawler.addRunner('notice',
        runner, HtmlCsRunner.standard.WCAG2AA, HtmlCsRunner.level.NOTICE);
      crawler.addRunner('warning',
        runner, HtmlCsRunner.standard.WCAG2AA, HtmlCsRunner.level.WARNING);
      crawler.addRunner('error',
        runner, HtmlCsRunner.standard.WCAG2AA, HtmlCsRunner.level.ERROR);
